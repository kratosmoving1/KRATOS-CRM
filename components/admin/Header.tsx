...
export default function Header({ onMobileMenuToggle, currentUser }: HeaderProps) {
  const router = useRouter()
  const { openModal } = useCreateModal()
  const [dropdownOpen, setDropdownOpen] = useState(false)
  const dropdownRef = useRef<HTMLDivElement>(null)
  const initials = (currentUser?.full_name ?? currentUser?.email ?? 'KM')
    .split(/[ @.]+/)
    .filter(Boolean)
    .slice(0, 2)
    ...
    <header className="flex h-16 items-center justify-between border-b border-slate-200/80 bg-white/95 px-4 shadow-[0_1px_0_rgba(15,23,42,0.02)] backdrop-blur md:px-6">
      ...
      <div className="relative max-w-xl flex-1">
        ...
        <input
          type="search"
          placeholder="Search…"
          className="w-full rounded-xl border border-slate-200 bg-slate-50/80 py-2.5 pl-9 pr-4 text-sm text-slate-900 placeholder-slate-400 outline-none transition focus:border-kratos focus:bg-white focus:ring-2 focus:ring-kratos/20"
        />
      </div>
      ...
      <div className="ml-4 flex items-center gap-2">
        ...
        <div ref={dropdownRef} className="relative">
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-kratos px-3.5 py-2.5 text-sm font-semibold text-slate-950 shadow-sm transition hover:-translate-y-px hover:shadow-md"
        ...
        <button className="relative rounded-xl p-2 text-slate-500 transition hover:bg-slate-100" aria-label="Notifications">
          <Bell size={18} />
          <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-kratos" />
        </button>

        <div
          className="flex h-9 w-9 items-center justify-center rounded-full bg-kratos text-xs font-semibold text-slate-950 shadow-sm select-none"
          title={currentUser?.full_name ?? currentUser?.email ?? 'Kratos'}
        >
          {initials}
        </div>
