...
function ChargeRowMenu({
  charge, onEdit, onDelete,
}: { charge: OpportunityCharge; onEdit: () => void; onDelete: () => void }) {
...
export default function ChargesSection({ charges, onAddCharge, onEditCharge, onDeleteCharge, deleting }: Props) {
  ...
  function requestDelete(chargeId: string) { setConfirmDeleteId(chargeId) }

  function confirmDelete() {
    if (confirmDeleteId) { onDeleteCharge(confirmDeleteId); setConfirmDeleteId(null) }
  }
                  ...
                  <tr key={charge.id} className={cn(
                    ...
                    <td className="py-3 pr-3">
                      ...
                      {charge.is_overridden && (
                        <p className="text-[10px] text-amber-600">Agent override</p>
                      )}
                    </td>
                    <td className="py-3 pr-3 text-xs text-slate-500">{chargeRateLabel(charge)}</td>
                    ...
                    <td className="py-3">
                      <ChargeRowMenu charge={charge} onEdit={() => onEditCharge(charge)} onDelete={() => requestDelete(charge.id)} />
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  <span className="font-medium">{confirmCharge.name}</span>
                  {' — '}{formatMoney(confirmCharge.total)}
                </p>
                <p className="mt-1 text-xs text-slate-400">
                  This will remove the charge and recalculate the estimate total.
                </p>
              </div>
              <button onClick={() => setConfirmDeleteId(null)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100">
                <X size={15} />
              </button>
            </div>
