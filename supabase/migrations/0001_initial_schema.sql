-- ============================================================
-- Kratos CRM — Initial Schema
-- ============================================================

-- Sales agents and other internal users
create table if not exists profiles (
  id uuid primary key references auth.users(id) on delete cascade,
  full_name text not null,
  email text not null unique,
  role text not null default 'junior_sales' check (role in (
    'admin','sales_manager','senior_sales','junior_sales','dispatcher','accountant','ops_manager'
  )),
  avatar_url text,
...
create table if not exists lead_sources (
  ...
  category text check (category in ('paid','organic','referral','repeat','other')),
  is_active boolean default true,
  created_at timestamptz default now()
);
...
create table if not exists opportunities (
  ...
  service_type text not null default 'local' check (service_type in (
    'local','long_distance','commercial','packing','storage','international'
  )),
  status text not null default 'new_lead' check (status in (
    'new_lead','contacted','quote_sent','accepted','booked','completed','cancelled','lost'
  )),
...
create index if not exists idx_opps_status on opportunities(status) where is_deleted = false;
create index if not exists idx_opps_created_at on opportunities(created_at);
create index if not exists idx_opps_booked_at on opportunities(booked_at);
create index if not exists idx_opps_completed_at on opportunities(completed_at);
create index if not exists idx_opps_sales_agent on opportunities(sales_agent_id);
create index if not exists idx_opps_lead_source on opportunities(lead_source_id);
create index if not exists idx_opps_service_date on opportunities(service_date);
...
do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'profiles' and policyname = 'auth users full access'
  ) then
    create policy "auth users full access" on profiles for all to authenticated using (true) with check (true);
  end if;
end $$;

do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'lead_sources' and policyname = 'auth users full access'
  ) then
    create policy "auth users full access" on lead_sources for all to authenticated using (true) with check (true);
  end if;
end $$;

do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'customers' and policyname = 'auth users full access'
  ) then
...
do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'opportunities' and policyname = 'auth users full access'
  ) then
    create policy "auth users full access" on opportunities for all to authenticated using (true) with check (true);
  end if;
end $$;

do $$ begin
  if not exists (
    select 1 from pg_policies where tablename = 'audit_log' and policyname = 'auth users full access'
  ) then
    create policy "auth users full access" on audit_log for all to authenticated using (true) with check (true);
  end if;
end $$;
...
begin
  ...
  -- avg profit per customer this month
  select
  ...
  from opportunities
  where is_deleted = false and sales_agent_id is null and status = 'new_lead';

  select count(*)::int into v_new_leads
  from opportunities
  where is_deleted = false and status = 'new_lead';
  ...
  select jsonb_build_object(
    ...
    'activity', jsonb_build_object(
      'today', jsonb_build_object(
        'leads', v_leads_today, 'quotesSent', v_quotes_today,
        'booked', v_booked_today, 'cancellations', v_cancel_today
      ),
      'week', jsonb_build_object(
        'leads', v_leads_week, 'quotesSent', v_quotes_week,
        'booked', v_booked_week, 'cancellations', v_cancel_week
      ),
      'month', jsonb_build_object(
        'leads', v_leads_month, 'quotesSent', v_quotes_month,
        'booked', v_booked_month, 'cancellations', v_cancel_month
      )
    ),
    'openItems', jsonb_build_object(
      'unassignedLeads',    v_unassigned_leads,
      'newLeads',           v_new_leads,
      'acceptedNotBooked',  v_accepted,
      'staleOpportunities', v_stale
    ),
    'salesLeaders', (
      ...
      join profiles p on p.id = sl.sales_agent_id
    ),
    'referralSources', (
      select coalesce(jsonb_agg(
        jsonb_build_object(
      ...
      from (
        ...
        where o.is_deleted = false
          and o.booked_at >= v_month_start
          and o.booked_at < v_month_end
        group by ls.name
        order by revenue desc
        limit 5
      ) rs
    ...
    'monthlyRevenue', (
      ...
      left join (
        select
          date_trunc('month', booked_at) as month_start,
          sum(total_amount) as revenue
        from opportunities
        where is_deleted = false and booked_at is not null
        group by 1
